...
Param(
    ...
    [string]$cdnSite, # => CDN SharePoint site "https://technoverthyd.sharepoint.com/"
...
Write-Host ($currentLocation + $releaseName + $releaseFolder + "\config\copy-assets.json")
$cdnConfig = Get-Content -Raw -Path ($currentLocation + $releaseName + $releaseFolder + "\config\copy-assets.json") | ConvertFrom-Json
$bundlePath = ($currentLocation + $releaseName + $releaseFolder + "\" + $cdnConfig.deployCdnPath) #Convert-Path $cdnConfig.deployCdnPath
